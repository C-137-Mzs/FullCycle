...
class Vehicle(RoadObject):
    ...
    """ Vehicle length [m] """
    WIDTH = 2.0
    # DEFAULT_INITIAL_SPEEDS = [18, 20]  # """ Range for random initial speeds [m/s]
    # lane：class StraightLane(AbstractLane):  speed_limit: float = 40, 没有时，才起作用。
    # 但是限速实际受到 # highway_env.py：def _create_road(self)：speed_limit=25
    MAX_SPEED = 40.
    ...
    def create_random(cls, road: Road,
        ...
        if speed is None:
            if lane.speed_limit is not None:
                ...
                speed = road.np_random.uniform(1.0 * lane.speed_limit, 1.0 * lane.speed_limit)
            else:
