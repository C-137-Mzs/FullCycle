...
class AbstractEnv(gym.Env):
    ...
    # def step(self, action: Action) -> Tuple[Observation, float, bool, dict]:  # 源代码
    #     """
    ...
    def step(self, action: Action) -> Tuple[Observation, float, bool, dict]:  # 修改后代码
        # print('envs.common.abstract => class AbstractEnv(gym.Env): def step(action):【racetrack and highway 都用】', action)  # racetrack and highway 都用
        if self.road is None or self.vehicle is None:
            raise NotImplementedError("The road and vehicle must be initialized in the environment implementation")
        self.steps += 1         # print('abstract: def act(): step:',  self.steps)  #
        #
        # if self.steps >= 200:    # 这个200步的设置在主程序中的 env.config["duration"]
        #     self.success = True

        self._simulate(action)

        obs = self.observation_type.observe()  # envs.common.observation:   class KinematicObservation(ObservationType):   FEATURES: List[str] = ['presence', 'x', 'y', 'vx', 'vy']
        reward = self._reward(action)   # Highway_env.py:def _reward(self, action: Action) -> float: return reward,lane
        terminal = self._is_terminal()
        info = self._info(obs, action)

        success = self.success

        return obs, reward, terminal, info, success
    ...
    def _simulate(self, action: Optional[Action] = None) -> None:
        ...
        for frame in range(frames):
            ...
            self.road.act()
            self.road.step(1 / self.config["simulation_frequency"])  # DQN_HEAD 中定义 4
            self.time += 1
