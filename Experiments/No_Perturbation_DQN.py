...
# env.config["punish_speed_range"] = config.punish_speed_range   #没用
...
#MEMORY_CAPACITY = 20        # 池子容量  200 * 20
...
while True:  # 原始程序
    ...
    while not done:
        ...
        #========================# 训练后 #===========================#
            ...
            # print(f"Before max_reward:{config.max_reward}")
            # print(f"Before reward:{config.reward}")
            # config.max_reward = max(config.max_reward,config.reward)
            # config.reward = config.reward if config.reward < config.max_reward else 2*config.max_reward-config.reward
            # print(f"After max_reward:{config.max_reward}")
            # print(f"After reward:{config.reward}")
        ...
        if dqn.position >= MEMORY_CAPACITY:  # 池子满了，开始训练
            ...
            if (config.trained_times == total_train_times + 1 ):
                ...
                for e in range(config.nreward):
                    ...
                    wb.save('02-DQN-C-reward_re.xlsx')  # 保存excel表
                #
                ...
                #     wb.save('02-DQN-C-tmep.xlsx')  # 保存excel表
                ...
                # 读取Excel文件
                xlsx_file = pd.read_excel('02-DQN-C-reward_re.xlsx', engine='openpyxl')
                # 假设数据位于Excel文件的第一个工作表的某一列中，并将其转换为float类型列表
                # 如果数据位于特定的列或行，请根据实际情况调整
                # 这里假设数据在第一列
                dqn_data = xlsx_file.iloc[:, 0].astype(float).tolist()
                print(dqn_data)
                total = len(dqn_data)
                dqn_num = 15
                dqn_matrix, iters_dqn = Draw.data_process(dqn_data, total, dqn_num)
                # FIG图像绘制部分
                # plt.xkcd()
                ...
                color_dqn = '#48D1CC'
                Draw.draw_line("DQN", dqn_matrix, color_dqn, iters_dqn)
                plt.xticks(fontsize=22)
                plt.yticks(fontsize=22)
                plt.xlabel('Episodes(×1000)', fontsize=32)
                plt.ylabel('Reward', fontsize=32)
                plt.legend(loc='upper left', fontsize=25)
                plt.title("Under No Attack - Highway", fontsize=34)
                plt.show()
                ...
                #
                # model_path = "model_fine_tuned_dqn/DQN_" + formatted_date_time + ".pth"
