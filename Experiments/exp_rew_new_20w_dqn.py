...
class DQN(object):
    ...
    def choose_action(self, s, e):  # s: 5*5矩阵
        ...
        x = torch.as_tensor(x, dtype = torch.float32, device = device) # cuda 计算

        if np.random.uniform() < 1 - e:  # - e
            actions_value = self.eval_net.forward(x)
            action = torch.max(actions_value, -1)[1].item()  # '-1'可视为‘1’，输出行最大值的索引，如果没有[1]就输出 最大值 和 索引
        else:
...
# model_path = 'model_rew/rew_new_20w_dqn_20250311_234949.pth'
#model_path = 'model_oar/oar_20w_dqn_20250308_033621.pth'
...
################################  开始训练  ###################################################
        if dqn.position >= MEMORY_CAPACITY:  # 池子满了，开始训练
            ...
            if (config.trained_times == total_train_times + 1 ):
                ...
                if not os.path.exists("model_rew"):
                    os.makedirs("model_rew")

                model_path = "model_rew/" + name + "_" + formatted_date_time + ".pth"
